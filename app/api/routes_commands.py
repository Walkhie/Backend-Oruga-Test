...
from app.core.orion_client import update_entity_state, create_entity, get_entities, delete_entity, get_entity_state_speed
...
def get_entity_state_route(entity_id:str):
    try:
        state = get_entity_state_speed(entity_id)
        if state is None:
